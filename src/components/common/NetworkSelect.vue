<template>
  <Listbox v-model="selectedNetwork" v-slot="{ open }">
    <div class="relative mt-1 z-40 text-sm">
      <ListboxButton
        class="px-3.5 py-2.5 rounded-2xl w-full flex justify-between items-center text-white"
        :class="[
          {
            'active_eth':
      ...
      >
        <ListboxOptions class="absolute -left-8 py-2 mt-1 focus:outline-none rounded-2xl bg-gray-700">
          <ListboxOption
          ...
          >
            <li
              :class="[
                selectedNetwork.id === network.id ? ' bg-gray-800 text-word-3' : 'bg-gray-700 text-word-4',
